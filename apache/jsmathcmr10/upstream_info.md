# Investigation Report: jsMath cmr10
...
| **Source path** | `jsmathcmr10/src/` |
| **Buildable** | No — no original design sources present |

The googlefontdirectory-hg monorepo (a git mirror of the original Google Code Mercurial repository) contains a `jsmathcmr10/src/` directory with only:
...
- **Directory**: `apache/jsmathcmr10/`
- **Files**: DESCRIPTION.en_us.html, jsMath-cmr10.ttf, METADATA.pb
...
The font is based on the Computer Modern Roman font (`cmr10`) — the primary upright roman font in Donald Knuth's Computer Modern family, used as the default text font in TeX/LaTeX documents. It was generated from MetaFont (.mf) bitmap sources using the `mftrace` tool.
