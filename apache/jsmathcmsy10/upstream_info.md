# Investigation Report: jsMath cmsy10
...
| **Source path** | `jsmathcmsy10/src/` |
| **Buildable** | No — no original design sources present |

The googlefontdirectory-hg monorepo (a git mirror of the original Google Code Mercurial repository) contains a `jsmathcmsy10/src/` directory with only:
...
- **Directory**: `apache/jsmathcmsy10/`
- **Files**: DESCRIPTION.en_us.html, jsMath-cmsy10.ttf, METADATA.pb
...
The font is based on the Computer Modern Math Symbols font (`cmsy10`), which provides mathematical symbols (operators, set notation, etc.) for TeX/LaTeX typesetting. It was generated from MetaFont (.mf) bitmap sources using the `mftrace` tool.
