...
from gflanguages import LoadLanguages, LoadRegions, LoadScripts
...
def test_LoadLanguages():
    with importlib_resources.as_file(DATA_DIR) as data_path:
        for langs in [LoadLanguages(), LoadLanguages(None), LoadLanguages(data_path)]:
...
def test_LoadScripts():
    with importlib_resources.as_file(DATA_DIR) as data_path:
        for scripts in [LoadScripts(), LoadScripts(None), LoadScripts(data_path)]:
            scripts = LoadScripts()
...
def test_LoadRegions():
    with importlib_resources.as_file(DATA_DIR) as data_path:
        for regions in [LoadRegions(), LoadRegions(None), LoadRegions(data_path)]:
            regions = LoadRegions()
            br = regions["BR"]
