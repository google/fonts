...
The original design sources for Lily Script One are preserved in the **googlefontdirectory-hg** Mercurial monorepo at commit `52f780bc9d197280a9f430574e179a5f233c56b6`, under the path `lilyscriptone/src/`.

### Source Files in googlefontdirectory-hg

- `LilyScriptOne-Regular.glyphs` -- Glyphs format source (buildable with gftools-builder)
- `LilyScriptOne-Regular-OTF.vfb` -- FontLab binary source (proprietary, not buildable with gftools-builder)
- `LilyScriptOne-Regular-TTF.sfd` -- FontForge SFD file (legacy, not buildable with gftools-builder)
- `LilyScriptOne-Regular.otf` -- compiled OTF binary, not a design source
- `METADATA_comments.txt` -- metadata file, not a design source

**The `.glyphs` file is gftools-builder compatible** and could be used to create an override config.yaml for reproducible builds.
...
Since both the librefonts repo and the googlefontdirectory-hg monorepo contain a `.glyphs` source file (`LilyScriptOne-Regular.glyphs`), an override `config.yaml` can be created in the google/fonts family directory for gftools-builder compatibility.
