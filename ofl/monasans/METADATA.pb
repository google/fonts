name: "Mona Sans"
designer: "Github, Deni Anggara"
license: "OFL"
category: "SANS_SERIF"
date_added: "2024-06-28"
fonts {
  name: "Mona Sans"
  style: "normal"
  weight: 400
  filename: "MonaSans[wdth,wght].ttf"
  post_script_name: "MonaSans-ExtraLight"
  full_name: "Mona Sans ExtraLight"
  copyright: "Copyright 2022 The Mona Sans Project Authors (https://github.com/github/mona-sans), with Reserved Font Name \"Mona\""
}
fonts {
  name: "Mona Sans"
  style: "italic"
  weight: 400
  filename: "MonaSans-Italic[wdth,wght].ttf"
  post_script_name: "MonaSans-ExtraLightItalic"
  full_name: "Mona Sans ExtraLight Italic"
  copyright: "Copyright 2022 The Mona Sans Project Authors (https://github.com/github/mona-sans), with Reserved Font Name \"Mona\""
}
subsets: "latin"
subsets: "latin-ext"
subsets: "menu"
subsets: "vietnamese"
axes {
  tag: "wdth"
  min_value: 75.0
  max_value: 125.0
}
axes {
  tag: "wght"
  min_value: 200.0
  max_value: 900.0
}
source {
  repository_url: "https://github.com/googlefonts/mona-sans"
  commit: "56384c90b986d431470303b4f7296585cb9c4515"
  files {
    source_file: "OFL.txt"
    dest_file: "OFL.txt"
  }
  files {
    source_file: "googlefonts/variable/MonaSans-Italic[wdth,wght].ttf"
    dest_file: "MonaSans-Italic[wdth,wght].ttf"
  }
  files {
    source_file: "googlefonts/variable/MonaSans[wdth,wght].ttf"
    dest_file: "MonaSans[wdth,wght].ttf"
  }
  branch: "main"
}
stroke: "SANS_SERIF"
minisite_url: "https://github.com/mona-sans"
