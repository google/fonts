# Noto Sans SC — Upstream Source Investigation
...
| Version | 2.004 |
| Onboarding PR | [google/fonts#5533](https://github.com/google/fonts/pull/5533) |
| Date | 2022-12-09 |

## Investigation Summary

Noto Sans SC is a regional subset of the Noto CJK mega-font family, targeting Simplified Chinese. The binary was taken from the `Sans2.004` tag at commit `523d033d`, specifically from `Sans/Variable/TTF/Subset/NotoSansSC-VF.ttf`. An 832-byte post-processing difference exists between the upstream binary and the shipped version, consistent with table rewriting during onboarding.
...
3. **Regional subsetting**: The unified CJK source contains glyphs for all regions (SC, TC, HK, JP, KR). Each regional variant is produced by selecting the appropriate glyph forms for that locale — Simplified Chinese uses locale-standard character forms that may differ from other regions for the same Unicode codepoints.

4. **Pre-built binaries**: The notofonts/noto-cjk repository distributes pre-built variable TTF files under `Sans/Variable/TTF/`. The `Subset/` subdirectory contains smaller regional subsets (e.g., `NotoSansSC-VF.ttf`) that are what Google Fonts ships.
