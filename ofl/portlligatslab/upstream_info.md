# Port Lligat Slab — Source Metadata Investigation
...
| **Source path** | `ofl/portlligatslab/src/` |
...
- `PortLligatSlab-Regular-OTF.vfb` — FontLab VFB (proprietary, not buildable with gftools)
- `PortLligatSlab-Regular-TTF.sfd` — FontForge SFD (open format, not supported by gftools-builder)
- `PortLligatSlab-Regular.otf` — compiled binary, not a design source
- `portligat-SLAB-google.jpg` — reference image
...
Designer: Tipo (Dario, tipo.net.ar). Script: Latin. Category: SERIF / DISPLAY (slab serif stroke).
